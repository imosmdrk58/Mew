...
export async function fetchLatestChapterManga(id) {
  try {
    const res = await fetch(
      `https://api.mangadex.org/manga/${id}/aggregate?translatedLanguage[]=en`
    );
    if (!res.ok) {
      return [];
    }
...
async function fetchAllChapter(id) {
  ...
  for (let item in volumes) {
    ...
    Object.values(volumes[item].chapters).map((chapter) =>
      volumeObject.chapterArray.push(chapter)
    );
...
async function fetchFileNames(id) {
  try {
    ...
    const fileName = result.data.relationships
      .filter((item) => item.type === "cover_art")
...
export async function fetchMostViewed() {
  const res = await fetch(
    "https://api.mangadex.org/manga?order[followedCount]=desc&availableTranslatedLanguage[]=en"
  );
  const mangaMostView = await res.json();
  const promises = mangaMostView.data.map((manga) =>
    fetchLatestChapterManga(manga.id)
  );
  const promisesTwo = mangaMostView.data.map((manga) =>
    fetchCoverMangas(manga.id)
  );
...
export async function fetchManga(
  ...
  try {
    ...
    const res = await fetch(
      `https://api.mangadex.org/manga?limit=${limit}&offset=${offset}&availableTranslatedLanguage[]=en${
        title !== "" ? `&title=${title}` : ""
...
export async function fetchRelatedMangaWithTags(mangaTagsObj, mangaId) {
  ...
  const includedTagIDs = tags.data
  ...
  const respond = await fetch(
    `https://api.mangadex.org/manga?includedTags=${includedTags}&availableTranslatedLanguage[]=en`
  );
  const data = await respond.json();
  // Filter out the current manga and get 5 related manga
  const mangaRelated = data.data
  ...
  const promisesTwo = mangaRelated.map((manga) =>
    fetchLatestChapterManga(manga.id)
  );
...
export async function fetchChapterImages(idChapter) {
  const respond = await fetch(
    `https://api.mangadex.org/at-home/server/${idChapter}`
  );
  const result = await respond.json();
  const host = result.baseUrl;
  const hash = result.chapter.hash;

  const promises = result.chapter.data.map(async (chapter) =>
    fetch(`${host}/data/${hash}/${chapter}`)
  );
  const images = await Promise.all(
    promises.map((promise) => promise.then((response) => response.url))
  );
  return images;
}
...
export async function searchManga(searchTerm) {
  const respond = await fetch(
    `https://api.mangadex.org/manga?title=${searchTerm}&availableTranslatedLanguage[]=en&limit=4`
  );
  const result = await respond.json();
...
export async function fetchAllReadChapter(
  email: string,
  mangaId: string,
  nextAuthUrl: string
) {
...
export async function fetchTagFilterWithID(filterArray: Array<string>) {
  try {
    ...
    const data = await respond.json();
    const includedTagIDs = data.data
      .filter((tag) => filterArray.includes(tag.id))
...
export async function fetchMangasWithFilter(
  filters: { [key: string]: any },
  tagIDs: Array<string>
) {
  try {
    let queryTagIDs = tagIDs
      .map((tagID) => `includedTags[]=${tagID}`)
      .join("&");
    const queryTagFilters = Object.keys(filters)
      .filter((key) => filters[key].toString() !== "any")
    ...
    if (!respond.ok) {
      throw new Error(
        `API request mangas with filter failed with status: ${respond.status}`
      );
    }
    const result = await respond.json();
    console.log(result);
